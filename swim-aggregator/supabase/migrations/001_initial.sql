...
create table exercises (
  ...
  -- FTS column; populated via trigger below (PG rejects to_tsvector in generated cols).
  search_tsv tsvector
);

create or replace function exercises_update_tsv() returns trigger language plpgsql as $$
begin
  new.search_tsv :=
    setweight(to_tsvector('english', coalesce(new.name, '')), 'A') ||
    setweight(to_tsvector('english', array_to_string(coalesce(new.aliases, '{}'), ' ')), 'B') ||
    setweight(to_tsvector('english', array_to_string(coalesce(new.instructions, '{}'), ' ')), 'C') ||
    setweight(to_tsvector('english', array_to_string(coalesce(new.focus_notes, '{}'), ' ')), 'C');
  return new;
end $$;

create trigger exercises_tsv_trg
  before insert or update on exercises
  for each row execute function exercises_update_tsv();
